...
void pio_usb_host_port_reset_start(uint8_t root_idx) {
  ...
  pio_sm_set_pins_with_mask64(pp->pio_usb_tx, pp->sm_tx, (0b00 << root->pin_dp),
                            (0b11ULL << root->pin_dp));
...
static void on_device_connect(pio_port_t *pp, root_port_t *root,
  ...
  pio_sm_set_pins_with_mask64(pp->pio_usb_tx, pp->sm_tx, (0b00 << root->pin_dp),
                            (0b11ULL << root->pin_dp));
  pio_sm_set_pindirs_with_mask64(pp->pio_usb_tx, pp->sm_tx, (0b11ULL << root->pin_dp),
                               (0b11ULL << root->pin_dp));

  busy_wait_ms(100);

  pio_sm_set_pindirs_with_mask64(pp->pio_usb_tx, pp->sm_tx, (0b00ULL << root->pin_dp),
                               (0b11ULL << root->pin_dp));
